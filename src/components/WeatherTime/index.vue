<template>
  <div
    ...
    @click.stop
  >
    <div class="time" @click.stop="toggleSiteStatus">
      <span class="hour">{{ timeData.hour ?? '00' }}</span>
      <span class="separator" :v-show="set.showSeconds">:</span>
...
const getWeatherData = async () => {
  if (!weatherKey) {
    return window.window.$message.warning('请配置天气 Key');
  }
  ...
  const tempLast = localStorage.getItem('lastWeatherData');
  if (tempLast) {
    lastWeatherData = JSON.parse(tempLast);
  } else {
    lastWeatherData = {
      data: {},
      lastFetchTime: 0
  ...
  if (timeDifference >= 5 * 60 * 1000) {
    ...
    if (adCodeResult.infocode !== '10000') {
      return window.$message.error('地区查询失败');
    }
...
const toggleSiteStatus = () => {
  status.setMenuStatus(status.menuStatus === 'menu' ? '' : 'menu');
};
