...
// 切换壁纸
const changeBackground = (type: number, reset = false) => {
  if (reset) {
    window.$dialog.warning({
      ...
      onPositiveClick: () => {
        ...
        window.$message.info('已恢复为默认壁纸，刷新后生效');
      }
    });
    return true;
  }
  backgroundType.value = type;
  window.$message.success(`已切换为${backgroundTypeArr[type].name}，刷新后生效`);
};
...
const setCustomCover = () => {
  if (identifyInput(customCoverUrl.value) === 'url') {
    ...
    window.$message.success('已切换为自定义壁纸，刷新后生效');
  } else {
    window.$message.error('请输入正确的网址');
  }
};
...
const resetSite = () => {
  window.$dialog.warning({
    ...
    onPositiveClick: () => {
      localStorage.clear();
      window.$message.info('站点重置成功，即将刷新');
      setTimeout(() => {
...
const backupSite = () => {
  try {
    ...
    // 备份完成
    window.$message.success('站点备份成功');
  } catch (error) {
    console.error('站点备份失败：', error);
    window.$message.error('站点备份失败');
  }
};
...
const recoverSite = async () => {
  try {
    ...
    if (!fileInput?.files?.length) {
      window.$message.error('请选择要恢复的备份文件');
      return false;
    }
    ...
    window.$dialog.warning({
      ...
      onPositiveClick: async () => {
        ...
        if (isSuccess) {
          window.$message.info('站点恢复成功，即将刷新');
          setTimeout(() => {
            window.location.reload();
          }, 1000);
        } else {
          window.$message.error('站点数据恢复失败，请重试');
        }
      },
      onNegativeClick: () => {
        recoverRef.value!.value = '';
      }
    });
  } catch (error) {
