...
// 自定义搜索引擎数据
const customEngineRef = ref<typeof NForm | null>(null);
...
// 更换搜索引擎
const changeSearchEngine = (key: string) => {
  // 获取元素
...
const customEngineClick = () => {
  if (set.customEngineUrl) {
    ...
    window.$message.info('无自定义数据，请配置');
