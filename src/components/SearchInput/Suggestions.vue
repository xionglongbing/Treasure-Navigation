...
// 搜索关键字类别
const searchKeywordType = ref<string>('text');
// 搜索建议数据
const searchSuggestionsData = ref<string[]>([]);
// 搜索建议元素
const specialallResultsRef = ref<HTMLElement | null>(null);
const allResultsRef = ref<HTMLElement | null>(null);
// 搜索建议高度
const suggestionsHeights = ref<number>(0);
// 接收搜索框内容
...
// 搜索框联想，使用防抖函数
const keywordsSearch = debounce((val: string) => {
...
// 响应键盘事件
const keyboardEvents = (keyCode: number, event: KeyboardEvent) => {
  try {
    // 获取元素
    ...
    if (keyCode === 38 || keyCode === 40) {
      ...
      if (mainInput && allResultsRef.value && searchSuggestionsData.value[0]) {
        const suggestionItems = allResultsRef.value.querySelectorAll<HTMLElement>('.s-result');
        if (suggestionItems.length > 0) {
          // 获取当前已聚焦的元素
          const focusedItem = document.querySelector('.s-result.focus');
          // 确定当前聚焦的元素在列表中的索引
          const currentIndex = Array.from(suggestionItems).indexOf(focusedItem as HTMLElement);
          // 移除所有元素的选中状态
    ...
    window.$message.error('出现问题，请尝试重置程序');
...
const changeSuggestionsHeights = () => {
  try {
    const allResultsHeight = allResultsRef.value?.offsetHeight || 0;
    const specialallResultsHeight = specialallResultsRef.value?.offsetHeight || 0;
    suggestionsHeights.value = specialallResultsHeight + allResultsHeight;
  } catch (error) {
