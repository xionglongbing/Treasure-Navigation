<template>
  ...
  <div
    :class="[
      'weather-time',
      status.siteStatus,
      status.siteStatus !== 'normal' && status.siteStatus !== 'focus' ? 'hidden' : null,
    ...
    <div
      ...
      @click.stop="
        status.setSiteStatus(
          status.siteStatus !== 'normal' && status.siteStatus !== 'focus' ? 'normal' : 'menu'
        )
      "
    >
...
// 时间数据
const timeData = ref<Partial<TimeData>>({});
...
const getWeatherData = async () => {
  if (!weatherKey) {
    return window.$message.warning('请配置天气 Key');
  }
  // 当前时间戳
